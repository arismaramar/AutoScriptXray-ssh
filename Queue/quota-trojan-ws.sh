#!/bin/bash
run_data() {
data=$(xray api statsquery --server=127.0.0.1:10001 | grep -C 2 $user | sed /"}"/d | sed /"{"/d | grep value | awk '{print $2}' | sed 's/,//g; s/"//g' | sort)
...
cekfile1=$(cat /etc/Anggun/cache/trojan-ws/$user | wc -l)
if [[ $cekfile1 -gt 0 ]]; then
echo > /dev/null
else
echo "0" > /etc/Anggun/cache/trojan-ws/$user
fi
}    
...
echo "$vbaru" > /etc/Anggun/cache/trojan-ws/$user
}

dataku=( `xray api statsquery --server=127.0.0.1:10001 | grep user | cut -d ">" -f 4 | sort | uniq`)
for user in "${dataku[@]}"
do
